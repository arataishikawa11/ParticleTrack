...
x_dynamics1 = np.array([[T*np.cos(theta), -T*np.sin(theta), 0.5*np.cos(theta)*T**2, -0.5*np.sin(theta)*T**2],
                      [T*np.sin(theta), T*np.cos(theta), 0.5*np.sin(theta)*T**2, 0.5*np.cos(theta)*T**2]]) # dynamics for z
...
for p in range(num_p):
    ...
    best_result = least_squares(func, x0, args=(M_z, b_z), max_nfev=2000)

    z_results.append(best_result.x)
...
for p in range(num_p):
    ...
    x_solution = least_squares(func, x_0_improved, args=(M_x, b_x), max_nfev=2000)
...
#         x_0_improved_Z = np.zeros(4+2*projections) # Initial guess to iinput in Z coupled matrix
#         x_0_improved_Z[2] = result[p,5]  # Use estimated y_0 from x calculation
